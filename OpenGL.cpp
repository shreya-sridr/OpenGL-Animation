...
glRotatef(theta[10],0.0, 1.0, 1.0);
...
glLoadIdentity();
glTranslatef(0, 0, 0.0);
glRotatef(theta[3], 0.0, 0.0, -0.8);
glGetFloatv(GL_MODELVIEW_MATRIX, rua_node.m);
glLoadIdentity();
glTranslatef(0, 0, 0.0);
glRotatef(theta[4], -1.5, 1.0, 0.0);
...
GLfloat mat_ambient[] = { 3.0, 0.0, 2.0, 1.0 };
...
glutCreateMenu(menu);
glutAddMenuEntry("Stretch", 0);
glutAddMenuEntry("One-legged prayer", 1);
glutAddMenuEntry("Warrior pose", 2);
glutAddMenuEntry("Reset", 3);
glutAddMenuEntry("Chair pose", 4);
glutAddMenuEntry("Arrow pose", 5);
